 ...
 * @see WP_Widget
 */
class izzy_Widget_Latest_Projects extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description'                 => __( 'Your site&#8217;s most latest Projects.', 'izzy' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'latest-projects', __( 'Latest Projects', 'izzy' ), $widget_ops );
		$this->alt_option_name = 'widget_latest_entries';
	}

	/**
	 * Outputs the content for the current Latest Projectss widget instance.
	 *
	 * @since 2.8.0
	 *
	...
	public function widget( $args, $instance ) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Latest Projects', 'izzy' );
		...
		/**
		 * Filters the arguments for the Latest Project widget.
		 *
		 * @since 3.4.0
        ...
        <ul>
			<?php foreach ( $r->posts as $latest_post ) : ?>
				...
				$title      = ( ! empty( $post_title ) ) ? __( strip_tags( $post_title ), 'izzy' ) : __( '(no title)', 'izzy' );
				?>
                <li>
	...
	/**
	 * Handles updating the settings for the current Latest Projects widget instance.
	 *
	 * @since 2.8.0
	 *
	...
	public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of projects to show:', 'izzy' ); ?></label>
        ...
        <p><input class="checkbox" type="checkbox"<?php checked( $show_date ); ?>
            ...
            <label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Display project date?', 'izzy' ); ?></label>
        </p>
		<?php
	}
}
